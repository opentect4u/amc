...
<?php
	if($_SERVER["REQUEST_METHOD"]=="POST"){
			...
			$sql = "SELECT `trf_dt`,
	                       ...
	                       `approval_status`,
	                       `created_by`,
	                       `created_dt` FROM td_damage_out
						 			    WHERE memo_no = '$item_code'
									    LIMIT 1";
			...
			if($report_result){
				...
				while($report_data = mysqli_fetch_array($report_result, MYSQLI_NUM)) {
								echo '<table style="padding: 6px;">
										...
										<th>Center Name</th>
										<th>Created By</th>';
								if($report_data[3]!= 1 || $_SESSION['access_type'] == 'AD'){
									...
									<td>'.$report_data[5].'</td>';
								if($report_data[3]!= 1 || $_SESSION['access_type'] == 'AD'){
									echo '<td><a href="damage_edit.php?memo_no='.$item_code.'" class = "edit_delete">EDIT</a>
														<a href="javascript:delete_damage(\''.$item_code.'\')" class ="edit_delete">DELETE</a>
														<a href="approve_damage.php?item_code='.$item_code.'" class = "edit_delete">Approve</a></td></tr>';
								}else{
									echo '</tr>';
								}
								echo '</table>';
