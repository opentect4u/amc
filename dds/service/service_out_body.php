<script type="text/javascript">
    ...
    $('#intro').on('change', '.trackAll', function(){
    	...
    	$('.preferenceSelect' + tempIndex).each(function(){
    		$('.preferenceSelect' + tempIndex).find('option[value ="' + this.value + '"]').attr('disabled', true);
    	});
    });
    ...
    $('#intro').on('click', '.addComp', function(){
    	...
    	highest++;
    	$('#adcmp' + currIndex).append('<tr><td> </td><td></td><td><?php require 'fetch_parts.php';?><select name="comp_no['+currIndex+']['+highest+']" class="input_select preferenceSelect'+currIndex+' trackAll" id="track'+currIndex+highest+'" style="width: 250px;" required><option>Select</option><?php while($parts = mysqli_fetch_assoc($parts_result)) {?><option value="<?php echo $parts['sl_no']?>"><?php echo $parts['parts_desc']?></option><?php }echo '<option value="0">No Parts</option>';?></select></td><td><input type="text" name="qty['+currIndex+']['+highest+']" value="1" class="input_select qty" style="width: 55px;" id="'+count+'" required></td><td></td><td></td></tr>');

    		$('.preferenceSelect' + currIndex).each(function(){
    			$('.preferenceSelect' + currIndex).find('option[value ="' + this.value + '"]').attr('disabled', true);
    		});
    });
	...
	function addRow(mc_qty, count){
		while(mc_qty > 0){
			counter = 0;
			$("#intro").append('<hr><table id="adcmp'+count+'"><tr><td><input type="text" name="mc_sl_no[]" class="input_select mc_sl_no" style="width:150px;"/></td><td><?php require 'fetch_problem.php';?><select name="prob[]" class="input_select" style="width: 150px;" required><?php while($pdata = mysqli_fetch_assoc($problem_result)){ ?><option value="<?php echo $pdata['problem_desc'];?>"><?php echo $pdata['problem_desc'];?></option><?php } ?></select></td><td><?php require 'fetch_parts.php';?><select name="comp_no['+count+']['+counter+']" class="input_select preferenceSelect'+count+' trackAll" id="track'+count+counter+'" style="width: 250px;" required><option>Select</option><?php while($parts = mysqli_fetch_assoc($parts_result)) {?><option value="<?php echo $parts['sl_no']?>"><?php echo $parts['parts_desc']?></option><?php }echo '<option value="0">No Parts</option>';?></select></td><td><input type="text" name="qty['+count+']['+counter+']" value="1" class="input_select qty" style="width: 55px;" id="'+count+'" required></td><td><input class="input_text" name="serviceBy[]" style="width: 100px;"></td><td><input type="button" style="padding:10; color:white; background-color:gray; width:50px; height:25px; font-size:25px;" value="+" class="addComp"/></td><td class="cnt" id="w'+count+'"></td></tr></table>');
			count++
			mc_qty--;		
		}		
	}
...
<div class="item_body" style="width: 850px;">
	...
	<br>
		...
		<table>
			...
			<tr><td>Ticket No</td>
				<td><input class="input_select" id="tkt_no" name="tkt_no" value="<?php echo $tkt_no;?>" readonly></td>
			</tr>
			...
			<tr>
				...
				<td><input class="input_select" name="mc_type" value="<?php echo $mc_type;?>" readonly></td>
			</tr>
			<tr><td>Submit By</td>
						...
						require 'fetch_tkt.php';
							...
							if (mysqli_num_rows($tkt_result) > 0) {
								while($tkt_data = mysqli_fetch_assoc($tkt_result)) {
									...
									//COUNTING TOTAL NO OF APPROVED MC QTY
									$sql = "SELECT COUNT(DISTINCT mc_sl_no) qty FROM td_service_out
																		    WHERE approval_status = 1 AND tkt_no =".$tkt_data['tkt_no']."";

									$result = mysqli_query($conn, $sql);									    

									$data = mysqli_fetch_assoc($result);

									if ($data['qty'] == $tkt_data['mc_qty']) {
										continue;
									}else{
...
<script type="text/javascript">
    ...
    $('#intro').on('change','.mc_sl_no', function(){
    	...
    	if($.inArray($(this).val(), global_arr) < 0) {
    		$(this).val('');
  ...
  $('#intro').on('change','.cnt', function(){
	   ...
	   $('.submit').click(function() {
		  ...
		  $('select').each(function(){
	   		$('select').find('option[value ="' + this.value + '"]').attr("disabled", false);
	   	  });
