...
<script src="js/delete_parts.js"></script>
	<title>SYNERGIC TRANSFER VIEW</title>
	<link rel="icon" href="../../favicon.ico">
				...
				<form name="report_form" action="" method="POST">
					<table>
						<tr>
							<td>Transfer No</td>
							<td><?php
								require 'fetch_transfer.php';
									echo '<select name="item_code" class="input_select" >';
									if (mysqli_num_rows($trf_data) > 0) {

										while($item_data = mysqli_fetch_assoc($trf_data)) {
											?>
												<option value='<?php echo $item_data["trf_no"];?>'>
														<?php echo $item_data['trf_no']; ?></option>
											<?php
										}
									}
								echo '</select>';
			...
			<div>
			  ...
			  <?php
			  	if(isset($_POST["mode"]) == "deletestock"){
					$trf_no = $_POST["trf_no"];
					$sqlupdate="DELETE FROM `td_transfer` WHERE trf_no = '$trf_no'";
					$resultupdate = mysqli_query($conn, $sqlupdate);

					$URL = $l_dds_view_transfer;
