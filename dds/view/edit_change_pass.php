...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_SESSION['username']!= "" && !empty($_SESSION['username']) && isset($_SESSION['username']) && $_SESSION['insert_flag']=="" && !empty($_SESSION['login_flag']) && isset($_SESSION['login_flag'])) {
				...
				$sqlupdate="UPDATE `login_master` SET `user_password`='".$new_password."' WHERE user_id='".$_SESSION['username']."' and user_password= '".$old_password."'";
			...
			if($resultupdate){
						...
						header('Location:'.$l_dds_change_pass.'');
