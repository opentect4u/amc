			...
			if($report_result){
				if (mysqli_num_rows($report_result) > 0) {
							...
							while($report_data = mysqli_fetch_array($report_result,MYSQLI_NUM)) {
								echo '<tr><td>'.$report_data[0].'</td><td>'.$report_data[1].'</td><td>'.$report_data[2].'</td><td>'.$report_data[3].'</td><td>'.$report_data[4].'</td><td>'.$report_data[5].'</td><td><a href="item_edit.php?item_code='.$item_code.'" class = "edit_delete">EDIT</a><a href="javascript:delete_item('.$report_data[0].')" class = "edit_delete">DELETE</a></td></tr>';
