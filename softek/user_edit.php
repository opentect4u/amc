<?php  
	...
	if($_SERVER["REQUEST_METHOD"]=="GET"){
		$login_code=trim_data($_GET["login_code"]);
			$retrieve_data="SELECT `user_id`,`user_password`,`user_type` FROM `mm_login_master` WHERE login_code='".$login_code."'";
...
<script type="text/javascript">
		function valid_form(){
	    	...
	    	return false;
	   		...
	   		document.getElementById('user_etype').innerHTML="Please Enter Account Type";
	   		return false;	
	   		}
	   		else{
	   		return true;
...
<div class="item_body">
	<h1>USER EDIT</h1>
	<form name="add_edit_form" method="POST" action="edit_user.php" onsubmit="return valid_form()">
	<table>
		<tr>
			<td>User Id</td>
		...
		<tr>
         ...
         <td><select name="user_type" class="input_select" required>
         		<option value="AD" <?php echo ($user_type == 'AD')? 'selected':'';?> >ADMIN</option>
         		<option value="STK" <?php echo ($user_type == 'STK')? 'selected':'';?> >USER</option>
         	</select>
		</td>
