...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
	$amc_code=trim_data($_GET["amc_code"]);
			$retrieve_data="select `amc_id`, `order_id`, `invoice_no`, `invoice_type`, `invoice_date`, `starting_date`, `duration`, `end_date`, `amount`, `tax`, `total_amount`, `updated_by`, `date_time` FROM `mm_amc_master`
			 WHERE amc_id='".$amc_code."'";
			 echo '<script>console.log("'.$amc_code.'")</script>';
			...
			if($report_result){
				if (mysqli_num_rows($report_result) > 0) {
							...
							while($report_data = mysqli_fetch_array($report_result,MYSQLI_NUM)) {
								$amc_id =  $report_data[0];
								$order_id =	 $report_data[1];
								$invoice_no = $report_data[2];
								$invoice_type = $report_data[3];
								$invoice_date = $report_data[4];
								$starting_date = $report_data[5];
								$duration = $report_data[6];
								$end_date = $report_data[7];
								$amount = $report_data[8];
								$tax = $report_data[9];
								$total_amount = $report_data[10];
...
<body>
	...
	<div class="nav_holder">
		...
		<?php require 'include/edit_amc.php';?>
        <?php require 'include/amc_body_edit.php';?>
